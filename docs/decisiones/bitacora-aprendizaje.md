...
| R-41 | ⬇️ **DE TRES A UNA, el 2026-09-15.** Tres fotografías que el cliente tachó de la galería el 2026-09-10 seguían siendo la cara de otras páginas. Al cambiar las imágenes de la portada y del restaurante —petición del cliente— han salido **dos de las tres**: `01-roof-top-atardecer` deja de ser la portada de `/restaurante/` y `06-alberca-roof-top` deja de ser su foto interior y la de la sección de experiencias de la portada. **Queda una:** `02-arco-playa`, que sigue siendo el banner de `/eventos/`. Y ya no hay escasez de banco: la galería tiene 160 fotos y `desde-galeria/` es el mecanismo para tomar una | Bajo | Cambiar el banner de `/eventos/` por una de la galería. Es una línea, pero nadie lo ha pedido y `/eventos/` es contenido inventado (R-30): conviene resolver el contenido antes que su ilustración |
